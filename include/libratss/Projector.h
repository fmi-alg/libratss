#ifndef LIB_RATSS_PROJECTOR_H
#define LIB_RATSS_PROJECTOR_H
#pragma once
...
template<typename T_FT>
PositionOnSphere Projector::positionOnSphere(const T_FT & z) const {
	if (z < 0) {
		return SP_LOWER;
	}
	else {
...
template<typename T_FT_ITERATOR>
PositionOnSphere Projector::positionOnSphere(T_FT_ITERATOR begin, const T_FT_ITERATOR & end) const {
...
template<typename T_FT_INPUT_ITERATOR, typename T_FT_OUTPUT_ITERATOR>
PositionOnSphere Projector::stProject(T_FT_INPUT_ITERATOR begin, const T_FT_INPUT_ITERATOR & end, T_FT_OUTPUT_ITERATOR out) const {
	using std::iterator_traits;
...
template<typename T_FT_INPUT_ITERATOR, typename T_FT_OUTPUT_ITERATOR>
void Projector::stInverseProject(T_FT_INPUT_ITERATOR begin, const T_FT_INPUT_ITERATOR & end, PositionOnSphere pos, T_FT_OUTPUT_ITERATOR out) const {
	using std::iterator_traits;
	using std::distance;
	using FT = typename iterator_traits<T_FT_INPUT_ITERATOR>::value_type;
	FT denom(1);
...
template<typename T_FT>
void Projector::stInverseProject(const T_FT & xp, const T_FT & yp, PositionOnSphere pos, T_FT & xs, T_FT & ys, T_FT & zs) const {
...
template<typename T_FT>
PositionOnSphere Projector::stProject(const T_FT & xs, const T_FT & ys, const T_FT & zs, T_FT & xp, T_FT & yp) const {
	if (zs < 0.0) {
		xp = xs / (1-zs); 
		yp = ys / (1-zs);
	}
...
template<typename T_FT>
void Projector::projectFromGeo(mpfr::mpreal lat, mpfr::mpreal lon, T_FT & xs, T_FT & ys, T_FT & zs, int precision) const {
	if (precision < 0) {
...
template<typename T_FT>
double Projector::projectFromGeo(mpfr::mpreal lat, mpfr::mpreal lon, T_FT &xs, T_FT &ys, T_FT &zs, double maxDist, int maxPrecision) const {
...
template<typename T_FT>
void Projector::projectFromSpherical(mpfr::mpreal theta, mpfr::mpreal phi, T_FT & xs, T_FT & ys, T_FT & zs, int precision) const {
	if (precision < 0) {
...
template<typename T_FT>
double Projector::projectFromSpherical(mpfr::mpreal theta, mpfr::mpreal phi, T_FT &xs, T_FT &ys, T_FT &zs, double maxDist, int maxPrecision) const {
	int minSqPrec = std::max<int>(maxPrecision, 256);
	mpfr::mpreal xf, yf, zf, xfs, yfs, zfs, sqd(std::numeric_limits<double>::max());
	{
...
template<typename T_FT>
void Projector::toSpherical(const T_FT & xs, const T_FT & ys, const T_FT & zs, double& theta, double& phi, int precision) const {
...
template<typename T_FT>
void Projector::toGeo(const T_FT & xs, const T_FT & ys, const T_FT & zs, double & lat, double & lon, int precision) const {
	mpfr::mpreal xr(internal::Conversion<T_FT>::toMpreal(xs, precision));
	mpfr::mpreal yr(internal::Conversion<T_FT>::toMpreal(ys, precision));
	mpfr::mpreal zr(internal::Conversion<T_FT>::toMpreal(zs, precision));
