...
std::string ProjectSN::toString(ProjectSN::SnapType st) {
	std::string result;
	#define PRINT_FIELD_NAME(__NAME) if ((st & __NAME) == __NAME) { result += #__NAME "|"; }
	
	PRINT_FIELD_NAME(ST_SPHERE)
	PRINT_FIELD_NAME(ST_PLANE)
	PRINT_FIELD_NAME(ST_PAPER)
	PRINT_FIELD_NAME(ST_PAPER2)
	PRINT_FIELD_NAME(ST_GUARANTEE_DISTANCE)
	PRINT_FIELD_NAME(ST_GUARANTEE_SIZE)
	PRINT_FIELD_NAME(ST_CF)
	PRINT_FIELD_NAME(ST_FX)
	PRINT_FIELD_NAME(ST_FL)
	PRINT_FIELD_NAME(ST_JP)
	PRINT_FIELD_NAME(ST_FPLLL_FIXED_N)
	PRINT_FIELD_NAME(ST_FPLLL)
	PRINT_FIELD_NAME(ST_BRUTE_FORCE)
